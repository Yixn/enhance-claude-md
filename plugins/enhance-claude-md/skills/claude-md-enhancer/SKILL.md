...
description: Analyzes, generates, and enhances CLAUDE.md files for any project type using best practices, modular architecture support, .claude/rules directory configuration, and tech stack customization. Use when setting up new projects, improving existing CLAUDE.md files, configuring .claude/rules directories, or establishing AI-assisted development standards.
...
The `.claude/rules/` directory provides a modular alternative to monolithic CLAUDE.md files. Rules are automatically discovered and loaded recursively at session start, making them ideal for larger projects and team environments where different areas require specialized guidance.

#### CLAUDE.md vs .claude/rules/ Comparison

| Aspect | CLAUDE.md | .claude/rules/ |
|--------|-----------|----------------|
| Structure | Single file | Multiple focused files |
| Best for | Small-medium projects | Larger projects, teams |
| Loading | Loads entirely | Loads recursively |
| Organization | One-file monolith | Topic-based directories |
| Maintenance | One person edits | Team ownership possible |
| Path targeting | Applies everywhere | Can target specific paths |

#### Memory Hierarchy

Claude Code loads instructions in the following precedence (highest to lowest priority):

1. **Enterprise policy** - Organization-wide rules (managed externally)
2. **Project memory** - `CLAUDE.md` or `.claude/CLAUDE.md` in project root
3. **Project rules** - `.claude/rules/*.md` files (loaded recursively)
4. **User memory** - `~/.claude/CLAUDE.md` (personal global instructions)
5. **User rules** - `~/.claude/rules/*.md` (personal rule files)
6. **Project memory local** - `CLAUDE.local.md` (not committed to git)

Higher priority sources override lower ones when instructions conflict.
...
Rules are loaded recursively, so subdirectories help organize complex projects.

#### Path-Specific Rules

Use YAML frontmatter to create rules that apply only to specific file paths:
...
These rules apply only when working with API and service files.
...
Path patterns support glob syntax. Rules without path frontmatter apply globally.

#### When to Use Rules vs CLAUDE.md

**Use .claude/rules/ when:**
- Project has 500+ lines of instructions
- Multiple team members need to own different areas
- Different rules apply to different parts of the codebase
- Path-specific rules are needed
- You want topic-based organization

**Stick with CLAUDE.md when:**
- Small-medium project with <500 lines of instructions
- General instructions apply everywhere
...
- **Descriptive filenames** - Names should indicate content at a glance
- **Keep files under 500 lines** - Split large topics into subtopics
- **Use subdirectories** - Organize by domain (frontend/, backend/, infra/)
- **Prefer positive instructions** - "Use Y" rather than "Never use X"
- **Team ownership** - Different teams maintain their own rule files
- **Lean content** - Focus on actionable constraints, not background

#### User-Level Rules

Personal rules in `~/.claude/rules/` apply across all your projects:

```
~/.claude/
├── CLAUDE.md           # Personal global instructions
└── rules/
    ├── coding-style.md # Your preferred code style
    ├── git-workflow.md # Your git preferences
    └── tools.md        # Tool preferences
```

Project rules take precedence over user rules, so project-specific needs always win.
...
```bash
# Create a shared rules directory
...
Claude Code handles circular symlinks gracefully.

#### Combining Rules with Hooks

Rules provide instructions (what Claude should do), while Hooks provide deterministic enforcement (what Claude must do). For critical requirements, use both:

- **Rules** - Document the requirement ("Always validate user input")
- **Hooks** - Enforce it automatically (PreToolUse hook that checks for validation)

This layered approach ensures requirements are both understood and enforced.
